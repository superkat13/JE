...
/**
 * One transparent context compiler shared by all Brain adapters.
 * It only renders data already present in Sage Core/twin memory/history/mode.
 * It does not add hidden personality rules or a provider-specific persona.
 */
class TwinContextRenderer(
    ...
    fun render(
        ...
        val active = memory.records.filter { it.active }
            .sortedWith(compareByDescending<com.pineapple.sageos2.memory.TwinMemoryRecord> { it.confidence }.thenByDescending { it.updatedAtEpochMs })
            .take(maxMemories)
        if (active.isEmpty()) appendLine("(none)")
        else active.forEach { record ->
        ...
        if (recent.isEmpty()) appendLine("(none)")
        else recent.forEach { entry ->
            val speaker = if (entry.speaker == ConversationSpeaker.OWNER) "OWNER" else "SAGE"
            appendLine("$speaker: ${entry.text}")
        }
    }.trim()

    private fun StringBuilder.appendList(label: String, values: List<String>) {
        if (values.isEmpty()) appendLine("$label: (none)")
        else {
            appendLine("$label:")
