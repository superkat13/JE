...
This adds a paste-only evidence parser on the Android side. It classifies boot properties
conservatively, never claims root from bootloader state, and never invokes adb/fastboot/su.
"""
...
/** Conservative parser for owner-collected Dell/ADB text. No commands are executed here. */
final class SageDellEvidence {
    ...
    static String valueFor(String raw, String key) {
        ...
        for (String sourceLine : raw.split("\\r?\\n")) {
            ...
            if (bracket >= 0 && line.endsWith("]")) {
                return line.substring(bracket + 3, line.length() - 1).trim();
            }
    ...
    static String report(String raw) {
        ...
        if ("0".equals(flashLocked)) {
            out.append("BOOTLOADER PROPERTY: reports unlocked/flash-unlocked.\\n");
        } else if ("1".equals(flashLocked)) {
            out.append("BOOTLOADER PROPERTY: reports locked.\\n");
        } else {
            out.append("BOOTLOADER PROPERTY: not established from pasted evidence.\\n");
        }

        if ("unlocked".equalsIgnoreCase(vbmetaState)) {
            out.append("VBMETA DEVICE STATE: reports unlocked.\\n");
        } else if ("locked".equalsIgnoreCase(vbmetaState)) {
            out.append("VBMETA DEVICE STATE: reports locked.\\n");
        } else {
            out.append("VBMETA DEVICE STATE: not established.\\n");
        }

        if (!verifiedBoot.isEmpty()) {
            out.append("VERIFIED BOOT: device reports ").append(verifiedBoot).append(".\\n");
        } else {
            out.append("VERIFIED BOOT: not established.\\n");
        }
    ...
    private static String show(String value) {
        return value == null || value.isEmpty() ? "not found" : value;
    }
}
'''
...
public final class SageDellEvidenceActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        evidence = new EditText(this);
        evidence.setHint("Paste Dell / ADB evidence here");
        evidence.setMinLines(8);
        evidence.setGravity(android.view.Gravity.TOP);
        evidence.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_FLAG_MULTI_LINE | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
        root.addView(evidence);

        Button parse = button("Interpret pasted evidence");
        parse.setOnClickListener(v -> interpret());
        root.addView(parse);

        Button clear = button("Clear evidence");
        clear.setOnClickListener(v -> { evidence.setText(""); result.setText("No evidence interpreted yet."); });
        root.addView(clear);

        result = text("No evidence interpreted yet.", 13);
    ...
    private void interpret() {
        String raw = evidence.getText().toString();
        result.setText(SageDellEvidence.report(raw));
        SageDiagnostics.appendEvent(this, "DELL EVIDENCE",
                "pasted_chars=" + raw.length() + " root_proven=false");
    }
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: dell_evidence_import_v1_29.py <reconstructed-source>")
    root = Path(sys.argv[1])
    java = root / "app/src/main/java/com/pineapple/sage"
    readiness = java / "SagePrivilegeReadinessActivity.java"
    redqueen = java / "SageRedQueenActivity.java"
    if not readiness.is_file() or not redqueen.is_file():
        raise SystemExit("Checkpoint 9 requires Checkpoint 8 privilege readiness")

    (java / "SageDellEvidence.java").write_text(PARSER)
    (java / "SageDellEvidenceActivity.java").write_text(ACTIVITY)

    manifest = root / "app/src/main/AndroidManifest.xml"
    replace_once(manifest, "    </application>",
        '        <activity android:name=".SageDellEvidenceActivity" android:exported="false" />\n    </application>',
        "Dell-evidence manifest insertion")
